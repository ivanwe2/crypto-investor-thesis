...
export const marketService = {
  getHistoricalKlines: async (
    symbol: string,
    interval: string = "1m",
    limit: number = 50,
  ): Promise<KlinesResponse> => {
    const response = await apiClient.get<KlinesResponse>(
      `${AppConfig.Endpoints.Markets}/${symbol}/klines`,
      {
  ...
  getOrderBookDepth: async (
    ...
    const response = await apiClient.get<OrderBookResponse>(
      `${AppConfig.Endpoints.Markets}/${symbol}/orderbook`,
      {
        params: { limit },
      },
    );

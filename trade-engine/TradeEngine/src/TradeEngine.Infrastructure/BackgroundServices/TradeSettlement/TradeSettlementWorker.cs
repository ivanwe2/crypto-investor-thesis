    ...
    ILogger<TradeSettlementWorker> logger) : BackgroundService
{
    ...
    {
        logger.LogInformation("🏦 Trade Settlement Engine started — {Lanes} parallel lanes.", SettlementQueue.LaneCount);

        // Launch one consumer per lane. Settlements for the same symbol are serialised
        // within a lane; different symbols settle concurrently across lanes.
        var laneTasks = Enumerable.Range(0, SettlementQueue.LaneCount)
            .Select(lane => ProcessLaneAsync(lane, stoppingToken));

        return Task.WhenAll(laneTasks);
    }

    private async Task ProcessLaneAsync(int lane, CancellationToken stoppingToken)
    {
        await foreach (var command in settlementQueue.GetReader(lane).ReadAllAsync(stoppingToken))
        {
            var sw = Stopwatch.StartNew();
            try
            {
                await SettleAsync(command, stoppingToken);
            }
            catch (Exception ex)
            {
    ...
    private async Task SettleAsync(TradeSettlementCommand command, CancellationToken stoppingToken)
    {
